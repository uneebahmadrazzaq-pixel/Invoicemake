...
test("Cosmetix Club is selectable and renders the supplied A4 invoice design", async () => {
  ...
  assert.match(editorSource, /function formatCosmetixAddress/);
  assert.match(editorSource, /index < 2 \? `<strong>/);
  assert.match(styles, /\.cosmetix-invoice\s*\{/);
  assert.match(styles, /width:\s*794px/);
  assert.match(styles, /min-height:\s*1123px/);
  assert.match(styles, /--cosmetix-pink:\s*#ef7c8e/);
  assert.match(styles, /--cosmetix-text:\s*#57585a/);
  ...
  assert.match(styles, /\.cosmetix-totals div:last-child\s*\{[^}]*border-bottom:\s*0/s);
  assert.match(styles, /\.cosmetix-footer\s*\{[^}]*background:\s*#e7e6e5/s);
  assert.match(styles, /\.cosmetix-payment\s*\{[^}]*border-left:\s*1px solid #d6d6d6/s);
  assert.match(styles, /\.cosmetix-contact\s*\{[^}]*border-top:\s*1px solid #d6d6d6/s);
