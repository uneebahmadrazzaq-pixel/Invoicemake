...
test("invoice editor exposes the requested workspace actions", async () => {
  ...
  assert.match(html, /id="downloadInvoice5mb"[\s\S]*?Download PDF Under 5 MB/);
...
test("invoice editor uses structured Bill To and Ship To address panels", async () => {
  ...
  assert.match(script, /downloadInvoice5mb\.addEventListener/);
  assert.match(script, /const maxPdfBytes = 5 \* 1024 \* 1024 - 1/);
