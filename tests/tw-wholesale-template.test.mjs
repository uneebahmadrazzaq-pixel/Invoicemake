...
test("TW Wholesale is selectable and renders a dedicated editable VAT invoice", async () => {
  ...
  assert.match(styles, /font-family:\s*"TW Roboto"/);
  assert.match(styles, /vetuk-roboto-regular\.ttf/);
  assert.match(styles, /vetuk-roboto-bold\.ttf/);
  assert.match(styles, /font-family:\s*"TW Roboto"[^;]*!important/);
  assert.match(styles, /font-synthesis:\s*none/);
  assert.match(editorSource, /document\.fonts\.load\('400 16px "TW Roboto"'\)/);
  assert.match(editorSource, /document\.fonts\.load\('700 16px "TW Roboto"'\)/);
  ...
  assert.match(styles, /\.tw-parties p\s*\{[^}]*color:\s*var\(--tw-charcoal\) !important;[^}]*font-size:\s*13\.333px/);
  assert.match(styles, /\.tw-payment h2\s*\{[^}]*color:\s*var\(--tw-muted\) !important;[^}]*font-size:\s*16px/);
  assert.match(styles, /\.tw-payment p\s*\{[^}]*color:\s*var\(--tw-charcoal\) !important;[^}]*font-size:\s*13\.333px/);
  assert.match(styles, /\.tw-terms h2\s*\{[^}]*color:\s*var\(--tw-muted\) !important;[^}]*font-size:\s*16px/);
  assert.match(styles, /\.tw-terms p\s*\{[^}]*color:\s*var\(--tw-charcoal\) !important;[^}]*font-size:\s*13\.333px/);
  ...
  assert.match(editorSource, /forceStyle\("\*",\s*\{[\s\S]{0,180}"font-family": '\"TW Roboto\", Roboto, Arial, Helvetica, sans-serif'/);
