...
test("Qogita UK is an editable A4 invoice matching the supplied reference", async () => {
  ...
  assert.match(styles, /\.qogita-uk-invoice\s*\{[\s\S]*font-size:\s*9px/);
  ...
  assert.match(styles, /\.qogita-company-grid h2,[\s\S]*font-size:\s*10\.5px !important/);
  assert.match(styles, /\.qogita-address-grid p \{ font-size:\s*9px !important; \}/);
  assert.match(styles, /\.qogita-products \{[^}]*font-size:\s*9px/);
  assert.match(styles, /\.qogita-transaction \{[^}]*margin-top:\s*72px[^}]*color:\s*#5f6776 !important/);
  assert.match(styles, /\.qogita-transaction strong \{ font-size:\s*10\.5px; \}/);
  assert.match(styles, /\.qogita-transaction b \{ font-size:\s*9px; \}/);
