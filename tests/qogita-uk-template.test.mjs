...
test("Qogita UK is an editable A4 invoice matching the supplied reference", async () => {
  ...
  assert.match(styles, /\.qogita-uk-invoice\s*\{[\s\S]*font-size:\s*9pt/);
  ...
  assert.match(styles, /\.qogita-company-grid h2,[\s\S]*font-size:\s*10\.5pt !important/);
  assert.match(styles, /\.qogita-address-grid p \{ font-size:\s*9pt !important; line-height:\s*1\.67 !important; \}/);
  assert.match(styles, /\.qogita-products \{[^}]*font-size:\s*9pt/);
  assert.match(styles, /\.qogita-company-grid p,[\s\S]*line-height:\s*1\.6 !important/);
  assert.match(styles, /\.qogita-products-section > h2 \{ margin-bottom:\s*14px; \}/);
  assert.match(styles, /\.qogita-totals dl \{ display:\s*grid; gap:\s*4px; \}/);
